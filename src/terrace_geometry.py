...
class MachiningFilterConfig:
    ...
    terrace_steps: int = 0
    # When True: skip Gaussian smoothing.
...
def filter_heightfield_for_machining(
    ...
    Apply all machining constraints to a heightfield in a deterministic sequence:
      ...
      5. Gaussian smoothing and terracing (smooth mode only).
    """
    if config is None:
    ...
    if config.terrace_mode:
        ...
        report.recommendations.append(
            "Terrace mode: Gaussian smoothing skipped. "
        ...
        # Step 5b: Terracing with fixed riser sigma
