...
def directional_step_convert(
    ...
    """Convert regions into directional stepped ridges via height quantization.

    Uses anisotropic blur to align iso-height contours with the grain
    direction, then quantizes the smoothed height values into discrete
    levels. This produces steps that naturally follow the grain.
    ...
    Args:
        ...
        step_width_px: Controls along-grain blur strength.
    ...
    # 1. Anisotropic blur: blur along grain direction, keep sharp across
    along_sigma = max(float(step_width_px) * 0.5, 2.0)
    across_sigma = 0.5
    smoothed = _directional_blur(hf, angle_rad, along_sigma, across_sigma)
    smoothed = np.clip(smoothed, 0.0, 1.0).astype(np.float32)

    # 2. Quantize height values (not coordinates), clip to 0.9999 to fix overflow
    n = max(int(n_steps), 2)
    stepped = np.floor(np.clip(smoothed, 0.0, 0.9999) * n) / (n - 1)
    stepped = np.clip(stepped, 0.0, 1.0).astype(np.float32)

    # 3. Apply mask if provided — blend with original at boundaries
    if mask is not None:
        mask = np.asarray(mask, dtype=np.float32)
        if mask.shape == hf.shape:
            if feather_px > 0:
                mask = gaussian_filter(mask, sigma=feather_px / 3.0)
                mask = np.clip(mask, 0.0, 1.0)
            stepped = hf * (1.0 - mask) + stepped * mask

    return np.clip(stepped, 0.0, 1.0).astype(np.float32)
